...
// let variabel = institutionManager.institutions[0].educations[0].schoolclasses[0].students[0];

let variabel = institutionManager.institutions[0].educations[0];
// console.log(variabel);
chooseEducation(variabel);

function chooseInstituion(institution){
    selectedStudent = null;
    selectedSchoolclass = null;
    selectedEducation = null;
    selectedInstituion = institution;
    outputList();
}

function chooseEducation(education){
    selectedStudent = null;
    selectedSchoolclass = null;
    selectedEducation = education
    selectedInstitution = education.institution;
    outputList();
}

function chooseSchoolClass(schoolclass) {
    selectedStudent = null;
    selectedSchoolclass = schoolclass;
    selectedEducation = schoolclass.education;
    selectedInstitution = selectedEducation.institution;
    outputList();
}

function chooseStudent(student) {
    selectedStudent = student;
    selectedSchoolclass = student.schoolclass;
    selectedEducation = selectedSchoolclass.education;
    selectedInstitution = selectedEducation.institution;
    outputList();
}

function outputList () {
    let outputElement = document.querySelector(editDataDisplayElement);
    outputElement.innerHTML = "";

    if(selectedStudent !== null){
        console.log("viser student");

    } else if(selectedSchoolclass !== null) {
        let html = "";
        selectedSchoolclass.students.forEach((student, studentindex) => {
            html += `<div data-indexpath ="${studentindex}">  ${student.name} </div>`;
        })
        outputElement.innerHTML = html;
        // events
        outputElement.querySelectorAll("div").forEach((element, index) => {
            
            element.addEventListener('click', () => {
                // console.log(element);
                // getClassIndexByReference();
                let student = selectedSchoolclass.students[index];
                chooseStudent(student);
                // console.log(selectedSchoolclass.students[index]);
            })
        })


    } else if(selectedEducation !== null) {
        console.log("viser skoleklasserne");

        let html = "";
        selectedEducation.schoolclasses.forEach((schoolclass, schoolclassindex) => {
            html += `<div data-indexpath ="${schoolclassindex}">  ${schoolclass.name} </div>`;
        })
        outputElement.innerHTML = html;
        // events
        outputElement.querySelectorAll("div").forEach((element, index) => {
            ...
            element.addEventListener('click', () => {
                // console.log(element);
                // getClassIndexByReference();
                let schoolclass = selectedEducation.schoolclasses[index];
                chooseSchoolClass(schoolclass);
                // console.log(selectedEducation.schoolclasses[index]);
            })
        })

        

    } else if(selectedInstituion !== null) {
        console.log("viser uddannelser");
    } else {
