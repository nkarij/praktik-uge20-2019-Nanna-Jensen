...
function removeButtonEvent(array){
    ...
    buttonElementsArray.forEach(buttonelement => {
        ...
        buttonelement.addEventListener('click', (event) => {
            ...
            let linkIndex = getIndex(buttonelement);
            // console.log(linkIndex);
...
function openFormEvent(array){
    ...
    buttonElementArray.forEach(button => {
            ...
            if(event.target == event.currentTarget) {
                ...
                let linkElementIndex = getIndex(button);
                // let linkElement = event.target.parentNode;
                // let linkElementIndex = linkElement.dataset.index;
                console.log(linkElementIndex);
                // console.log(linkElementInnerHTML);
                // her skal jeg så kunne redigere object.name og skrive det nye til DOMen.
                //  kan jeg isolere et item i mit array?
                console.log(array);
                let selectThisToEdit = array[linkElementIndex];
                
                clonedForm.children[1].innerHTML = selectThisToEdit.name;
                // let selectedExistingName = selectThisToEdit.name;
                // ryd datacontainer og indsæt formen;
                clearDataContainer();
                editDataContainer.appendChild(clonedForm);
                // editFormEvent(selectThisToEdit);
                    // hent form input og skriv det til DOM'en.
                submitEditedData(array, selectThisToEdit);
            }
        });
    }) 
}
